...
def train_and_evaluate(X_train, X_test, y_train, y_test):
    ...
    # LabelEncoder for XGBoost: -1→0, 0→1, 1→2
    le = LabelEncoder()
    y_train_enc = le.fit_transform(y_train)
    y_test_enc  = le.transform(y_test)
    joblib.dump(le, "models/label_encoder.pkl")

    # Sample weights for XGBoost only
    from sklearn.utils.class_weight import compute_sample_weight
    xgb_sample_weights = compute_sample_weight("balanced", y_train_enc)
    ...
    models = {
        ...
        "KNN": (
            KNeighborsClassifier(
                n_neighbors=9,
                weights="distance"
            ), True, False),   # ← no sample_weight for KNN
        ...
        "XGBoost": (
            ...
            ), False, True),   # ← use_sample_weight=True
    }
    ...
    for name, (model, use_scaled, use_sw) in models.items():
        Xtr = X_train_s if use_scaled else X_train.values
        Xte = X_test_s  if use_scaled else X_test.values
        is_xgb = (name == "XGBoost")

        if is_xgb:
            model.fit(Xtr, y_train_enc, sample_weight=xgb_sample_weights)
    ...
    joblib.dump(
        joblib.load(f"models/{best_name.replace(' ','_').lower()}.pkl"),
